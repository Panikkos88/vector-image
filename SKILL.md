...
- Auto-router v1 is now the default user path. It computes the Palette ladder once, accepts Palette only for small clean palettes with a strong edge signal, and otherwise routes to Region. Local and Cloud BOC default/no-query runs select Palette/k=3 at 2.60% edge RMSE, 0.27% MAE, 0.7% hot pixels, 55 paths, and about 15,835 nodes. Local and Cloud shaded-test default/no-query runs select Region because the palette guard rejects k=14/core residual 15.6. Cloud revision `vector-accuracy-studio-00009-k8j` is deployed and browser-tested. Next target: close the remaining BOC gap to the Vector Magic reference (about 2.41% edge / 55 paths) without increasing complexity, then broaden the router test set.
...
5. For the BOC/KOINO flat-logo path, tune compact boundary candidates around `tight-corners-s12..s18` to close 2.60% -> about 2.41% edge RMSE while keeping 55 paths and a reasonable node budget.
