...
- Thin-stroke recovery v1 is implemented and locally proofed, but the gain is small. It targets the post-routing outline-shield gap by detecting high-chroma small-area colors made of thin components, rebuilding those strokes/dots from a source-derived sub-pixel membership field, replacing only same-color thin SVG paths, and accepting the candidate only if the existing difference guard improves edge error without hot/path/node regression. On `bench-outline-shield`, it selected `thin-strokes-wide` for yellow `#ecb82f` and improved edge RMSE 9.78% -> 9.72%, hot 1.8% -> 1.7%, paths 31 -> 18, nodes 1372 -> 2543. Smoke checks held BOC/fine-text/metal; dark-glow thin-stroke skipped but current local dark-glow measured 3.32% edge / 68 paths instead of the earlier accepted 1.72% / 50, so investigate dark-glow drift before deploy. See `research/thin-stroke-recovery-v1.md`.
- Palette boundary simplifier v1 is implemented and locally proofed. It uses accurate rejected Palette boundary candidates as sources, refits their closed path endpoints with smooth and endpoint-line simplifier variants, and promotes only guarded results. On `bench-outline-shield`, it selected `boundary-simplifier:tight-corners-s12-c065:line-a30`; boundary edge improved 9.78% -> 5.36% with hot pixels 1.8% -> 0.7%, 31 paths, nodes 1372 -> 4116. With thin-stroke recovery after it, final outline measured 5.32% edge / 0.26% MAE / 0.7% hot / 18 paths / 5248 nodes. BOC, fine-text, and metal held; dark-glow still shows the pre-existing local drift at 3.32% / 68 paths. See `research/palette-boundary-simplifier-v1.md`.
- GitHub/public research pass `research/github-vectorization-research-2026-06-28.md` confirms the next outline lever should be subpixel/topology recovery and local vector-parameter optimization, not a generic tracer swap. Most useful sources: `JinfanYang/SubpixelDeblurring` for anti-aliased clip-art deblurring/topology recovery, `BachiLi/diffvg` plus LIVE/SGLIVE for measured vector optimization, VTracer/Potrace for corner/turn policies, and ImageTracerJS as the known baseline/ceiling. Recommended next sequence: quick unbounded Palette candidate probe, then ROI coverage-aware corner reconstruction around the two shield tips.
- Real-world logo seed pack v1 adds 24 varied internal validation PNGs under `app/assets/benchmark-realworld/`, with a manifest and source/composed SVGs under `research/real-logo-seed-pack/`. It covers flat marks, thin curves, dense details, transparent input, dark glow, low-res hard edges, gradient panels, multi-color flat marks, and metallic/text stress cases. Most source marks are from Simple Icons CC0 with brand/trademark caveats; use internally only. Generate/update with `tools/gather-real-logo-seed-pack.js`.
- The real-world logo seed pack has been run locally and on Cloud (`vector-accuracy-studio-00020-c4z`) with exact local/Cloud metric parity. Reports live in `research/real-logo-seed-pack/local-realworld-results-2026-06-29.*`, `cloud-realworld-results-2026-06-29.*`, and `local-vs-cloud-realworld-2026-06-29.md`. Batch average: 2.73% edge RMSE, 1.30% hot pixels, 14.7 paths. Strong flat-logo results confirm the current Palette route is healthy; the measured weak areas are transparent input routing (`telegram-transparent`), dark/glow hot pixels (`dark-apple-gloss`, `tiktok-dark-glow`, `react-atom`), low-res hard-edge upsampling (`x-lowres-black`), and metallic/tonal content (`metallic-wordmark-generated`).
- Guarded dark-glow Palette-vs-Region bake-off v1 runs a Palette challenger only for eligible dark-glow/gloss Auto routes that would otherwise use Region, then chooses by measured edge/hot/complexity metrics. Local and Cloud revision `vector-accuracy-studio-00023-htl` match: `dark-apple-gloss` improves from Region 4.40% edge / 7.4% hot to Palette 3.12% edge / 2.0% hot at 49 paths. The guard rejects `tiktok-dark-glow` because Palette would be 5.94% edge / 14.1% hot vs Region 3.80% / 2.2%, rejects `metallic-wordmark-generated` because edge does not improve, and leaves `telegram-transparent`, `bench-outline-shield`, and `boc-logo-small` unchanged. See `research/dark-glow-bakeoff-v1.md`.
- Guarded dark-glow High-detail bake-off v1 runs a High-detail challenger only after the normal Medium Auto result lands on Region with a strong dark-glow tonal signal. Local and Cloud revision `vector-accuracy-studio-00024-qb6` match: `tiktok-dark-glow` improves from 3.80% edge / 2.2% hot / 45 paths to 3.41% / 1.8% / 58 paths; `react-atom` improves from 4.09% / 4.6% / 11 paths to 2.95% / 0.9% / 18 paths. It does not run on Medium Palette winners like `dark-apple-gloss`, avoiding the forced-High Apple regression. See `research/dark-glow-detail-bakeoff-v1.md`.
- Guarded React tonal-band complexity lane v1 extends the dark-glow proof loop. The tonal-band pass now evaluates `baseline`, `fine`, and `broad` variants and has a `moderate-clean-win` budget for extra paths/nodes only when edge improves and hot pixels do not rise. The High-detail bake-off also has a strong-visual-win budget so High+tonal can be accepted. Local and Cloud revision `vector-accuracy-studio-00025-f26` match: `react-atom` improves from the prior High-only 2.95% edge / 0.9% hot / 18 paths to 2.69% / 0.9% / 45 paths. `tiktok-dark-glow`, `dark-apple-gloss`, and BOC held. See `research/dark-glow-tonal-variant-guard-v1.md`.
...
research/thin-stroke-recovery-v1.md
research/palette-boundary-simplifier-v1.md
research/github-vectorization-research-2026-06-28.md
research/real-logo-seed-pack/real-logo-seed-pack-2026-06-29.md
research/real-logo-seed-pack/local-realworld-results-2026-06-29.md
research/real-logo-seed-pack/cloud-realworld-results-2026-06-29.md
research/real-logo-seed-pack/local-vs-cloud-realworld-2026-06-29.md
research/dark-glow-bakeoff-v1.md
research/dark-glow-detail-bakeoff-v1.md
research/dark-glow-tonal-variant-guard-v1.md
```
...
5. For the BOC/KOINO flat-logo path, edge RMSE now matches the VM reference at about 2.41% with 55 paths. Do not optimize BOC edge first. Dark-glow tonal banding plus guarded Palette-vs-Region bake-off is now a measured win on `dark-apple-gloss`; guarded High-detail bake-off improves Region-held `tiktok-dark-glow`; guarded High+tonal improves `react-atom` to 2.69% edge. The next dark/glow work should build smarter local many-flat-band tonal geometry, not blanket-route all dark logos to Palette or High and not just add more global thresholds.
